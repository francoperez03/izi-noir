...
export function parseCircuitFunction(
  fn: Function,
...
function parseStatement(node: AcornNode): Statement | null {
...
function parseExpression(node: AcornNode): Expr | null {
  switch (node.type) {
    case 'Identifier':
    ...
    case 'LogicalExpression': {
      ...
      const left = parseExpression(node.left as AcornNode);
      const right = parseExpression(node.right as AcornNode);
